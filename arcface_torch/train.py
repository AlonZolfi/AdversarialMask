...
from arcface_torch.config import config as cfg
from partial_fc import PartialFC
import backbones
from arcface_torch.utils.utils_callbacks import CallBackVerification, CallBackLogging, CallBackModelCheckpoint
...
def main(args):
    ...
    world_size = 1
    rank = 0
    dist_url = "tcp://{}:{}".format('127.0.0.1', '1234')
    dist.init_process_group(backend='gloo', init_method=dist_url, rank=rank, world_size=world_size)
    local_rank = 'cpu'
    ...
    if args.resume:
        try:
            backbone_pth = os.path.join(cfg.output, "arcface_resnet100.pth")
